...
from models.augmented.quine import Auxiliary, GraphAuxiliary, SequentialAuxiliary, Vanilla
...
from .quine_trainer import AuxiliaryTrainer, GraphAuxiliaryTrainer, SequentialAuxiliaryTrainer, VanillaTrainer
...
def trainer(config: Dict, model: torch.nn.Module, param_data: torch.nn.Module, dataloaders: List[torch.utils.data.DataLoader], device: torch.device):
    ...
    if isinstance(model, (Classical, LinearModel)):
        ...
        return GraphAuxiliaryTrainer(config, param_data, dataloaders, device).run_train()
